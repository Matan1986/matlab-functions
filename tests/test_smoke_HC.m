function test_smoke_HC()
% Smoke test for HC ver1/HC_main.m
% Verifies: file exists, can be parsed, no syntax errors

    script_path = fullfile('HC ver1', 'HC_main.m');
    
    % Test 1: File exists
    assert(exist(script_path, 'file') == 2, ...
        'HC_main.m should exist');
    ...
    try
        ...
        error('Failed to read HC_main.m: %s', ME.message);
    end
    ...
    fprintf('✓ HC_main.m smoke test passed\n');
end
