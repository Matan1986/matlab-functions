function test_smoke_MT()
% Smoke test for MT ver2/MT_main.m
% Verifies: file exists, can be parsed, no syntax errors

    script_path = fullfile('MT ver2', 'MT_main.m');
    
    % Test 1: File exists
    assert(exist(script_path, 'file') == 2, ...
        'MT_main.m should exist');
    ...
    try
        ...
        error('Failed to read MT_main.m: %s', ME.message);
    end
    ...
    fprintf('✓ MT_main.m smoke test passed\n');
end
