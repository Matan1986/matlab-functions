% fit_script_ver_sin2_product_manual_save.m
...
%% 1) Define directory
baseDir = 'L:\My Drive\Quantum materials lab\Analysis Lab measurments\Magnetic Intercalated TMD\Co1_3TaS2\MG 131\MG131 FIB3 In Plane Rotator full vs partially in series zfAMR and high res 5 deg\zfAMR at 11T high res 5deg inc 4K 3\fitting';
...
open(figPath);
gcf_fig = gcf;  % get the handle to the opened figure
...
lines = flip(lines);  % to match legend order
...
%% 6) Prepare storage
n = numel(lines);
coeffs = zeros(n,3);  % a1,c1,c2
gofs   = zeros(n,4);  % SSE, R2, AdjR2, RMSE

%% 7) Define the fittype for product of two sines
ft = fittype( ...
    sprintf('a1*sin(%d*b*pi/180*x + c1).*sin(%d*b*pi/180*x + c2)', fold1, fold2), ...
    'independent', 'x', ...
    'coefficients', {'a1','c1','c2'}, ...  % three coefficients
    'problem', 'b');
...
for k = 1:n
    ...
    % --- Smart initial guess (3 values) ---
    amp_guess = (max(y) - min(y)) / 2;
    sp = [amp_guess, 0, 0];  % [a1, c1, c2]

    % --- Fit ---
    [f, g] = fit(x, y, ft, 'StartPoint', sp, 'problem', fixedB);

    % --- Store results ---
    coeffs(k, :) = [f.a1, f.c1, f.c2];
    ...
    legend(figName, sprintf('product folds=[%d,%d]', fold1, fold2), 'Location', 'northeast');

    % --- Overlay annotation ---
    annStr = sprintf('product folds=[%d,%d]\nSSE=%.3f\nR^2=%.3f\nRMSE=%.3f', ...
        fold1, fold2, g.sse, g.rsquare, g.rmse);
    axNew = gca;
    xL = axNew.XLim; yL = axNew.YLim;
    xPos = xL(2) - 0.02*(xL(2) - xL(1));
    yPos = yL(1) + 0.05*(yL(2) - yL(1));
    text(xPos, yPos, annStr, 'BackgroundColor', 'white', 'EdgeColor', 'black', ...
        'FontSize', 8, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom', 'Interpreter', 'none');
...
varNames = {'a1','c1','c2','SSE','R2','AdjR2','RMSE'};
