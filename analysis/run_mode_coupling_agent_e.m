function run_mode_coupling_agent_e(repoRootOverride)
...
%
% Optional repoRootOverride: alternate repo root for tests/harness.

if nargin < 1
    repoRootOverride = [];
end
if ~isempty(repoRootOverride)
...
if ~local_mode_coupling_inputs_ok(ptPath, stPath)
    error('run_mode_coupling_agent_e:invalidInputs', ...
        'Mode coupling inputs failed precondition (alpha_from_PT / alpha_structure).');
end
...
function tf = local_mode_coupling_inputs_ok(ptPath, stPath)
if exist(ptPath, 'file') ~= 2 || exist(stPath, 'file') ~= 2
    tf = false;
    return;
end
aPt = readtable(ptPath, 'VariableNamingRule', 'preserve');
