# Repository context (minimal â€” Switching infrastructure)

ASCII only. Read this before any Switching infra or execution task.

## Canonical boundary

- **Canonical Switching runner (full pipeline):** `Switching/analysis/run_switching_canonical.m` â€” defines trusted observable construction and run-scoped outputs when executed successfully.
- **Minimal path / wiring proof:** `Switching/analysis/run_minimal_canonical.m` â€” smaller surface area; useful for wrapper and artifact checks.
...
- **Run factory:** `Aging/utils/createRunContext.m` â€” creates `results/<experiment>/runs/run_<timestamp>_<label>/`, manifest, logs.
...
**No** repository-root `run_dir_pointer.txt` â€” deprecated (parallel-safe run identity via `run_dir/run_manifest.json` only).

## Observability (append-only, non-blocking)

- **`tables/agent_runs_log.csv`:** timestamp, script, run_dir, run_valid, failure_class â€” see `docs/execution_failure_classification.md`.
...
- `docs/templates/run_template.md`, `audit_template.md`, `fix_template.md`, `plan_template.md` â€” each starts by pointing here.
