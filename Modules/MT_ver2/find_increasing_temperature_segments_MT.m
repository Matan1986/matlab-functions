function [segments_increasing_temp_cell] = find_increasing_temperature_segments_MT(Timems, filtered_temp, min_segment_length_increasing_temp, TH, min_temp_change, min_temp_time_window_change, temp_rate, stabilization_window, delta_T)
    % Initialize cell array for segments
    segments_increasing_temp_cell = {};
            ...
            if in_segment_temp && filtered_temp(i) >= TH - delta_T
                ...
                if stable
                    ...
                    if (i - segment_start_temp) >= min_segment_length_increasing_temp
                        ...
                        segments_increasing_temp_cell{end + 1, 1} = [segment_start_temp, i]; % Add vertically
                    end
                end
            end
        end
    ...
    if in_segment_temp && (length(filtered_temp) - segment_start_temp) >= min_segment_length_increasing_temp
        segments_increasing_temp_cell{end + 1, 1} = [segment_start_temp, length(filtered_temp)]; % Add vertically
    end
end
