function plot_resistivity_warming_segments( ...
    resistivity_warming_tables, ...
    rounded_unique_field_max_values, ...
    plotChannels, ...
    plan_measured,showAngleUI)

% plot_resistivity_warming_segments
% ---------------------------------------------------------
% Plot rho(T) during WARMING, per field and per enabled preset channel.
% UNIQUE angles only (stable), merged across segments.
% Large temperature jumps are broken by NaNs.
%
% UI:
...
if isempty(enabledKeys)
    warning('plot_resistivity_warming_segments:NoChannels', ...
        'No enabled channels in plotChannels (preset). Nothing to plot.');
    return;
end

% ---- options ----
maxAllowedTempJump_K = 2;
angleRoundDeg        = 0;   % 0 = integer degrees

% ---- loop over fields ----
for f = 1:numel(resistivity_warming_tables)

    field_table = resistivity_warming_tables{f};
    ...
    if angleRoundDeg == 0
        anglesKey = round(anglesRaw);
    else
        anglesKey = round(anglesRaw, angleRoundDeg);
    end

    [anglesUnique, ia] = unique(anglesKey, 'stable'); %#ok<ASGLU>
    ...
    for iC = 1:numel(enabledKeys)
        ...
        figName = sprintf('%s %s  Warming  B = %.2f T', ...
        ...
        for ii = 1:num_angles_u
            ...
            dT = diff(Tall);
            breakIdx = find(dT > maxAllowedTempJump_K);

            Tplot = Tall;
            Rplot = Rall;
            for k = flip(breakIdx(:)')
                Tplot = [Tplot(1:k); NaN; Tplot(k+1:end)]; %#ok<AGROW>
                Rplot = [Rplot(1:k); NaN; Rplot(k+1:end)]; %#ok<AGROW>
            end
        ...
        % ---- labels ----
        title(sprintf('%s %s  Warming  B = %.2f T', ...
        ...
        if showAngleUI
            % ---- UI: angle selection (two columns) ----
            uicontrol('Style','text','Position',[10 560 80 14], ...
                'String','Angles','BackgroundColor','w','FontSize',6);

            nAngles = num_angles_u;
            nRows   = ceil(nAngles/2);

            colX  = [5 60];
            rowY0 = 540;
            rowH  = 14;

            checkBoxes = gobjects(nAngles,1);
