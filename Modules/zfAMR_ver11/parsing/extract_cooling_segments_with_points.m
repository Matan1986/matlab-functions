function cooling_points_table = extract_cooling_segments_with_points( ...
    Timems, cooling_tables, TemperatureK, unique_rounded_smoothed_angle_deg, temp_values, delta_T)
% extract_cooling_segments_with_points  
...
%   temp_values may be any length; columns T1, T2, …, TN will be created.
    ...
    cooling_points_table = cell(size(cooling_tables));
    ...
    for i = 1:numel(cooling_tables)
        table      = cooling_tables{i};
        angles     = table.Angle;
        new_table  = table;
        ...
        cooling_points_table{i} = new_table;
    end
end
