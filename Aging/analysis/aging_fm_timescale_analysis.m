...
writeAgingBaselineTauSidecarFm(runDir, cfg, fmTauTbl);
...
function writeAgingBaselineTauSidecarFm(runRoot, cfg, fmTauTbl)
% Write machine-readable tau disclosure sidecar next to tau_FM_vs_Tp.csv (B003-W1).
sr = aggregateSourceRunForSidecar(fmTauTbl);
dsId = string(cfg.datasetPath);
...
vals = [
    "FM_ABS_CURVEFIT"; ...
    "CURVEFIT_ZOO with buildEffectiveFmTau: half_range_seconds primary when half_range ok else tau_effective_seconds = 10^median(log10 trusted components)"; ...
    "FM_abs"; ...
    "tw"; ...
    "Aging/analysis/aging_fm_timescale_analysis.m"; ...
    "tau_FM_vs_Tp.csv"; ...
    sr; ...
    "LINEAGE_METADATA_HARDENED_PENDING_F7S"; ...
    "per_Tp_summary"; ...
    "seconds"; ...
    "buildEffectiveFmTau uses half_range primary when tau_half_range_status ok else median log10 of trusted logistic_log_tw stretched_exp half_range pool"; ...
    "tau_logistic_half_seconds,tau_stretched_half_seconds,tau_half_range_seconds"; ...
    "Per-row tau_consensus_methods in tau_FM_vs_Tp.csv is half_range_primary or comma methods per buildEffectiveFmTau"; ...
    dsId; ...
    "GRID_DISCLOSURE_REF: Tp 30 and Tp 34 lack tw=3 versus Tp 14-26 per aging_dip_fm_tw_inventory_01 PARTIAL_GRID"; ...
    "FM_abs ABS_ONLY magnitude collapsed from consolidation; not signed FM dynamics"; ...
    "tau_FM_vs_Tp.csv"
    ];
sideTbl = table(mf, vals, 'VariableNames', {'metadata_field', 'value'});
save_run_table(sideTbl, 'tau_FM_vs_Tp_sidecar.csv', runRoot);
end
