...
writeAgingBaselineTauSidecarDip(run_output_dir, datasetPath, tauTbl);
...
function writeAgingBaselineTauSidecarDip(runRoot, datasetPath, tauTbl)
% Write machine-readable tau disclosure sidecar next to tau_vs_Tp.csv (B003-W1).
sr = aggregateSourceRunForSidecar(tauTbl);
dsId = string(datasetPath);
if strlength(strtrim(dsId)) == 0
    dsId = "UNRESOLVED";
end
mf = [
    "tau_domain"; ...
    ...
    "units"; ...
    "builder_rule"; ...
    "trusted_component_tau_fields"; ...
    "consensus_methods"; ...
    "source_dataset_id"; ...
    "grid_disclosure"; ...
    "sign_or_magnitude_disclosure"; ...
    "output_artifact"
    ];
vals = [
    "DIP_DEPTH_CURVEFIT"; ...
    "CURVEFIT_ZOO with buildConsensusTau: tau_effective_seconds = 10^median(log10 of trusted positive logistic_log_tw stretched_exp half_range components per row"; ...
    "Dip_depth"; ...
    "tw"; ...
    "Aging/analysis/aging_timescale_extraction.m"; ...
    "tau_vs_Tp.csv"; ...
    sr; ...
    "REQUIRES_DATASET_PATH_AND_DIP_BRANCH_RESOLUTION"; ...
    "per_Tp_summary"; ...
    "seconds"; ...
    "buildConsensusTau selects trusted positive tau_logistic_half_seconds tau_stretched_half_seconds tau_half_range_seconds then median log10"; ...
    "tau_logistic_half_seconds,tau_stretched_half_seconds,tau_half_range_seconds"; ...
    "Per-row tau_consensus_methods column in tau_vs_Tp.csv lists methods feeding effective tau for that Tp"; ...
    dsId; ...
    "GRID_DISCLOSURE_REF: Tp 30 and Tp 34 lack tw=3 versus Tp 14-26 per aging_dip_fm_tw_inventory_01 PARTIAL_GRID"; ...
    "Dip_depth is a signed consolidation column; negative cells possible; disclose sign context when interpreting"; ...
    "tau_vs_Tp.csv"
    ];
sideTbl = table(mf, vals, 'VariableNames', {'metadata_field', 'value'});
save_run_table(sideTbl, 'tau_vs_Tp_sidecar.csv', runRoot);
end

function txt = aggregateSourceRunForSidecar(tbl)
...
mask = strlength(strtrim(sr)) > 0 & ~ismissing(sr);
sr = sr(mask);
if isempty(sr)
    txt = "UNRESOLVED";
    return;
end
txt = strjoin(unique(sr, 'stable'), ' | ');
end
