...
figure('Name', sprintf('%s %s %s difference', plan_measured_str, resistivity_type, meas_type), 'Position', [100, 100, 1000, 600]);
hold on;
...
for f = 1:length(resistivity_diff_tables)
    ...
    plot(angles, resistivity_diff, '-o', 'DisplayName', sprintf('%g T', fields(f)), 'Color', colors(color_indices(f), :));
end

hold off;
title(sprintf('%s %s (%s(%dK)-%s(%dK)) /< %s > [%%]', plan_measured_str, meas_type, resistivity_component_str,temp_low, resistivity_component_str,temp_high,devide_resistivity_component_str), 'FontSize', 14);
