...
obsGlobal = table( ...
    repmat("relaxation", 4, 1), repmat(string(sampleName), 4, 1), nan(4, 1), ...
    ["Relax_beta_global"; "Relax_tau_global"; "Relax_t_half"; "Relax_initial_slope"], ...
    [globalObs.Relax_beta_global; globalObs.Relax_tau_global; globalObs.Relax_t_half; globalObs.Relax_initial_slope], ...
    ["unitless"; "s"; "s"; "signal_per_log10s"], repmat("observable", 4, 1), repmat(string(runId), 4, 1), ...
    'VariableNames', {'experiment', 'sample', 'temperature', 'observable', 'value', 'units', 'role', 'source_run'});
ok = sliceTbl.fit_ok;
obsPerT = table();
for i = find(ok).'
    rows = table( ...
        repmat("relaxation", 4, 1), repmat(string(sampleName), 4, 1), repmat(sliceTbl.Temp_K(i), 4, 1), ...
        ["Relax_beta_T"; "Relax_tau_T"; "Relax_t_half"; "Relax_initial_slope"], ...
        [sliceTbl.param_beta(i); sliceTbl.param_tau(i); sliceTbl.Relax_t_half(i); sliceTbl.Relax_initial_slope(i)], ...
        ["unitless"; "s"; "s"; "signal_per_log10s"], repmat("observable", 4, 1), repmat(string(runId), 4, 1), ...
